...
# -----------------------------------------------------------------
echo -e "\n1. Reading active products using 'alice' (Should succeed)..."
curl -s -u "alice:123" "http://localhost:4004/odata/v4/test/Products" | jq '.'

# -----------------------------------------------------------------
# 2. RBAC MUTATION RESTRICTION TEST (Bob - Authenticated User)
# -----------------------------------------------------------------
echo -e "\n2. Attempting to create a product using 'alice' (Should fail with 403 Forbidden)..."
...
# -----------------------------------------------------------------
echo -e "\n4. Verifying persistence with advanced query parameters ($filter + $expand) using 'alice'..."
...
curl -s -u "alice:123" "http://localhost:4004/odata/v4/test/Products?\$filter=SKU%20eq%20'${NEW_SKU}'&\$expand=orderItems" | jq '.'
